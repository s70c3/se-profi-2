...
@app.post("/create_vm/", response_model=VirtualMachineResponse)
async def create_vm(vm_request: VirtualMachineRequest) -> VirtualMachineResponse:
    ...
    return VirtualMachineResponse(result="OK", host_id=target_server["server_id"])

@app.get("/servers_utilization/", response_model=List[Dict[str, float]])
async def read_servers_utilization():
    return get_servers_utilization()

@app.get("/server_utilization/{server_id}", response_model=Dict[str, float])
